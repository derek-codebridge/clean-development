...
- The automated suite ran 146 tests: 145 passed, zero failed, and the native-Windows execution test was skipped on macOS/Linux. It covers read-only planning, every session choice, exact project-file persistence, disabled-project transitions, owner-bound Claude environment files, OpenCode's additive host merge, runtime ownership, and package lifecycle behavior.
- Coverage on macOS arm64/Node 26.8.2 was 87.76% lines, 78.94% branches, and 91.88% functions. The final 100-pair no-op benchmark measured 57.96 ms added median and 67.28 ms added p95, below the provisional 75 ms p95 gate.
...
- macOS 26.6.2 arm64 with Node 24.20.0 passed check, the 145/0/1 suite, installed-package gate, all four real-tool smokes, the 6/6 fixture lab, and the dependency audit. Evidence is retained in `/Users/admin/clean-development-v020-acceptance-20260919-r5/evidence/` in the `clean-development-macos26` VM.
- Debian GNU/Linux 13 ARM64 passed check, the 145/0/1 suite, and the installed-package gate on exact Node 20.12.2 and 20.19.2. Cargo/npm smoke and four applicable fixture cases also passed; Go and uv were unavailable in that VM, so the complete four-tool smoke result comes from macOS. Evidence is retained in `~/clean-development-v020-acceptance-20260919-r5/evidence/` in the `debian13-arm64` Lima VM.
- A fresh, isolated `CODEX_HOME` on the separate macOS `test` account contained only its copied login and the setup-owned v0.2.0 configuration at launch. Codex CLI 0.154.0 with `gpt-5.6-terra` at high reasoning inherited `session-only`, resolved the managed uv shim and cache, completed all 914 Auditex tests offline, left the checkout clean, and created neither `.venv` nor `.pytest_cache`. Evidence is retained in `/Users/Shared/clean-development-v020-model-20260919-r5/evidence/`; the active Codex desktop session was not used or closed.
- Grok Build 1.0.34 repeated the same current-package acceptance in `clean-development-macos26`: all 914 Auditex tests passed offline through the managed uv shim/cache, the checkout remained clean, and no local virtual environment or pytest cache appeared. Evidence is retained in `/Users/admin/clean-development-v020-model-20260919-r5/evidence/`. AGY 1.2.7 remained installed and its launcher/version smoke passed, but the SSH test process did not inherit the GUI account's authentication, so it is not counted as a current model-backed acceptance.
